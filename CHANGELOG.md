...
- New `update-image-hdv2` action — the AI HelpDesk V2 (HDV2) counterpart to `update-image`. Updates a container image for an HDV2 workload within a workspace via duploctl: `type=service` (alias `eks`) targets `appservice` and `type=lambda` targets `hd_lambda`. Takes `name`, `image`, and one of `workspace`/`workspace_id` (falling back to `DUPLO_WORKSPACE`/`DUPLO_WORKSPACE_ID`); requires only `DUPLO_HOST`/`DUPLO_TOKEN`, so it works against integrated portals and standalone AI HelpDesks alike. ECS is not yet supported (no HDV2 backend endpoint; tracked in DUPLO-43548).
